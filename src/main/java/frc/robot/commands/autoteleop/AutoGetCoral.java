...
public class AutoGetCoral extends SequentialCommandGroup {
    public AutoGetCoral(AlignmentConfig config, CoralPivot coralPivot, GroundIntakePivot groundIntakePivot, Elevator elevator) throws FileVersionException, IOException, ParseException {
        PathPlannerPath path = PathPlannerPath.fromPathFile(config.pathName());
        Command pathFindingCommand =
            AutoBuilder.pathfindThenFollowPath(path, AutoTeleopConstants.coralStationAlignmentConstraints);

        addCommands(
            new ParallelCommandGroup(
                pathFindingCommand,
                new IntakeCoral(coralPivot, groundIntakePivot, elevator)
            )
        );
    }
}
