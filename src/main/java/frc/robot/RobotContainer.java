...
public class RobotContainer {
    ...
    private void configureButtonBindings() throws FileVersionException, IOException, ParseException {
        ...
        for (AlignmentConfig config : reefCoralAlignmentConfigs) {
            buttonBox.button(config.button()).and(buttonBox.button(l1Button)).onTrue(new AutoPlaceCoral(config, Level.L1, coralPivot, groundIntakePivot, elevator));
            buttonBox.button(config.button()).and(buttonBox.button(l2Button)).onTrue(new AutoPlaceCoral(config, Level.L2, coralPivot, groundIntakePivot, elevator));
            buttonBox.button(config.button()).and(buttonBox.button(l3Button)).onTrue(new AutoPlaceCoral(config, Level.L3, coralPivot, groundIntakePivot, elevator));
            buttonBox.button(config.button()).and(buttonBox.button(l4Button)).onTrue(new AutoPlaceCoral(config, Level.L4, coralPivot, groundIntakePivot, elevator));
        }
        for (ReefAlgaeAlignmentConfig config : reefAlgaeAlignmentConfigs) {
            buttonBox.button(config.button1()).and(buttonBox.button(config.button2())).onTrue(new AutoGetReefAlgae(config, coralPivot, groundIntakePivot, elevator));
        }
        for (AlignmentConfig config : coralStationAlignmentConfigs) {
            buttonBox.button(config.button()).onTrue(new AutoGetCoral(config, coralPivot, groundIntakePivot, elevator));
        }
        buttonBox.button(processorAlignmentConfig.button()).onTrue(new AutoProcessAlgae(processorAlignmentConfig, coralPivot, groundIntakePivot, elevator));
    }
